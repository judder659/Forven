...
export const SETTINGS_SUBSECTIONS: SettingsSubsection[] = [
  ...
  { id: 'lab-research', area: 'lab', label: 'Research', description: 'Idea sources, candidate screening, and the held-back data test.' },
...
export const SETTINGS_MANIFEST: SettingsEntry[] = [
  ...
  },
  {
    ...
    description:
      'Fold swarm-level selection bias into the Deflated Sharpe trial count: agents try many strategy variants per idea-cluster (family x asset) and only survivors reach the gauntlet, so the DSR trial count is multiplied by (1 + same-cluster siblings that failed). OFF = correct for per-strategy optimizer trials only.',
  ...
  },
  {
    ...
    description:
      'Window for counting same-cluster siblings that failed in the DSR swarm-trials factor. 0 = unbounded (count the full graveyard).',
    usedBy: ['forven.gauntlet.deflated_sharpe'],
  },
  ...
  // Research (delegated to ResearchSettingsPanel — persisted via saveSection('research', {research_settings: ...}))
  {
    ...
    description: 'Idea sources, the held-back data test, and other research-panel configuration.',
    usedBy: ['forven.api_core', 'forven.agents.tools_research', 'forven.strategy_creation'],
  },
  ...
  },
  {
    ...
    description: 'Backtest window for the autopilot evolution validation matrix (per-context, capped ~30k bars). 0 = inherit the Default backtest window.',
  ...
  },
  {
    ...
    description:
      'Autonomous strategy-creation tasks queued per UTC day. Each task writes one idea and builds strategies from it — the main driver of AI call volume (0-500). Ideas you submit do not count.',
    usedBy: ['forven.strategy_creation', 'forven.throughput_policy'],
    advanced: true,
  },
  {
    id: 'research.strategy_creation_max_in_flight',
    label: 'Strategy-creation tasks in flight',
    unit: 'tasks',
    default: 2,
    type: 'number',
    area: 'system',
    subsection: 'system-throughput',
    backendSection: 'research',
    backendPath: 'research_settings.strategy_creation_max_in_flight',
    description: 'How many strategy-creation tasks may be pending or running at once before the scheduler waits (1-20).',
    usedBy: ['forven.strategy_creation'],
    advanced: true,
  },
