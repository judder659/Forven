...
def get_ai_dropzone_context():
    ...
    return {
        ...
        "workflow": [
            ...
            "4. POST /api/strategies/intake/register-file with {\"file_path\": \"<absolute path>\", \"session_id\": \"<optional>\"} → strategy_id + stage (quick_screen if certified; otherwise archived with an untestable_reason — fix the file and register again).",
        ...
        "gotchas": [
            ...
            "Never read future bars (.shift(-1) etc.). Registration runs a lookahead probe and archives leaky strategies as untestable; implausible metrics (e.g. Sharpe > 10) auto-reject.",
            ...
            "When the research holdout is on (GET /api/robustness/holdout/{strategy_id} shows the cutoff), every backtest, optimization and walk-forward ends at the cutoff — a window past it shifts back, keeping its length — and get_local_ohlcv returns bars before it. The held-back period is for ONE test per candidate before paper, spent automatically the moment every other paper check is green (usually as the last robustness result lands, else on a promote attempt); the gate answers 'Held-back test pending' until the verdict lands (poll that GET; a PASS promotes on its own, a FAIL is final). It is never re-run for the same params and each strategy family gets a few per quarter, so finalize params BEFORE the last robustness run, and don't spend variants hoping for a second draw.",
        ],
        "sessions": {
