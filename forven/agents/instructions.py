...
4. Use the current BaseStrategy interface and registration template. Import from forven.strategies.base, export STRATEGY_CLASS and TYPE_NAME, and use deterministic, stateless signal logic. generate_signal returns the latest scalar Signal; generate_signals uses the engine's vectorized contract. Use closed-bar information with no future shifts or future-fitted features.
...
6. Validate with permitted lint/sandbox tools, then register_strategy with the real hypothesis_id and assigned crucible_id when provided. A source file or lint success is not a registered candidate. Confirm the returned Strategy Container ID and parent link before reporting creation.
